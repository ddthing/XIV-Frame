    ...
    <DocumentPage
      ...
      sections={[
        ...
        },
        {
          ...
          children: (
            <>
              ...
              <p>처음에는 <strong>사진 추가 → 레이아웃 선택 → 사진별 구도 조정 → 시그니처 조정 → 저장</strong> 순서로 사용하면 됩니다. 레이아웃을 먼저 고르면 미리보기의 빈 슬롯에 사진을 채울 수도 있습니다. 원본 비율이 기본이며, X 타임라인용 16:9와 2:1도 선택할 수 있습니다. 각 단계의 실제 조작은 <Link href="/ko/blog">가이드</Link>에서 화면 흐름에 맞춰 설명합니다.</p>
            </>
          ),
        },
        {
        ...
        },
        {
          ...
          children: (
            <>
              <p>처음에는 사진을 추가하고 레이아웃을 고른 다음 합성·시그니처·저장을 순서대로 진행하는 것이 가장 안정적입니다. 배경 제거처럼 기기 메모리를 사용하는 작업은 모바일에서 한 장씩 처리하고, 저장은 PNG를 우선 사용하되 X의 5MB 제한을 넘으면 불투명 배경에서 고화질 JPEG로 자동 최적화합니다.</p>
        ...
        },
        {
          ...
          children: (
            <FeatureCoverageTable
              intro="2026년 8월 26일 현재 검수본을 기준으로 실제 편집 화면에서 확인한 기능과 한계입니다. 운영 배포본의 상태는 배포 후 다시 확인하며, 브라우저·기기별 메모리와 파일 상태에 따라 처리 시간은 달라질 수 있습니다."
              areaLabel="영역"
              supportLabel="지원 기능"
              notesLabel="사용 기준"
              rows={[
                { area: '입력 이미지', support: 'AVIF · BMP · GIF · JPG · PNG · WebP', notes: '한 장 50MB 이하, 사진 최대 16장. 큰 이미지는 브라우저에서 편집용으로 최적화될 수 있습니다.' },
                { area: '레이아웃', support: '가로 분할 · 세로 분할 · 2×2 바둑판 · 3×3 · 4×4', notes: '2×2 바둑판은 3~4장(3장일 때 한 칸 비움), 3×3·4×4 격자는 각각 9장·16장을 사용하며 X 타임라인·원본 비율·2:1과 간격·테두리·배경을 조정합니다.' },
