      ...
      sections={[
        {
          ...
          children: (
            <>
              <p>XIV Frameは、ファイナルファンタジーXIV（FF14）のスクリーンショットを1枚の完成画像にまとめるブラウザベースの編集ツールです。複雑な画像編集ソフトを開かずに、複数の画像を配置し、キャラクター名やサーバー名、ロゴを追加して保存できます。</p>
              <ul>
                <li>最大16枚のスクリーンショットを追加して順番を変更できます。2×2の4枠グリッドは3～4枚、3×3・4×4グリッドはそれぞれ9枚・16枚を使います。</li>
                <li>分割・縦並び・グリッド・強調・均等配置のレイアウトとキャンバス比率を選べます。</li>
        ...
        },
        {
          ...
          children: (
            <>
              ...
              <p>まず画像を追加します。画像1枚なら自動で分割されないため、レイアウトを選ばずに署名を調整して保存できます。複数画像をまとめる場合は<strong>画像を追加 → レイアウトを選ぶ → 各画像を調整 → 署名を調整 → 保存</strong>の順に進めます。先にレイアウトを選び、プレビューの空きスロットに画像を入れることもできます。元の比率を初期設定にし、Xタイムライン用16:9と2:1も選べます。<Link href="/ja/blog">ガイド</Link>もこの順番で、エディターに表示されるラベルに沿って説明しています。</p>
            </>
          ),
        },
        {
        ...
        },
        {
          ...
          children: (
            <>
              <p>画像1枚なら分割せず署名と保存へ進み、複数画像をまとめる場合は画像を追加してからレイアウトを選ぶと安定します。モバイルで背景削除を使う場合は1枚ずつ処理します。PNGを優先し、Xの5MB制限を超える不透明な結果は高画質JPEGに自動最適化します。</p>
        ...
        },
        {
          ...
          children: (
            <FeatureCoverageTable
              intro="2026年8月31日に確認したレビュービルドを基準に、実際の編集画面で確認した機能と制限です。公開版の状態はリリース後に再確認します。処理時間はブラウザ、端末のメモリ、元ファイルによって変わります。"
              areaLabel="領域"
              supportLabel="対応機能"
              notesLabel="利用基準"
              rows={[
                { area: '入力画像', support: 'AVIF · BMP · GIF · JPG · PNG · WebP', notes: '1ファイル50MB以下、写真は最大16枚。編集用の長辺は画像枚数に応じて4096・3072・2048・1536pxに最適化されます。' },
                { area: 'レイアウト', support: '横・縦分割 · 2×2グリッド · 強調 · 均等配置 · 3×3 · 4×4', notes: '2×2グリッド、強調、3列・3行均等は3～4枚、4列・4行均等は4枚を使います。3枚の2×2グリッドでは1枠が空き、3×3・4×4はそれぞれ9枚・16枚を使います。Xタイムライン・元の比率・2:1と間隔・枠・背景（ホワイト・ブラック・ライトグレー・透明・カスタム）を調整でき、2:1で画像1枚なら標準・円・ハート・星も選べます。' },
                ...
                { area: '保存・保管', support: 'PNG・JPEGダウンロード', notes: 'PNGを優先し、Xの5MB制限を超える不透明な結果は高画質JPEGに自動最適化します。スクリーンショットはサーバーに保存しません。' },
              ]}
            />
          ),
        },
