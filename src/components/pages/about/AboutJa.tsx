    ...
    <DocumentPage
      ...
      sections={[
        ...
        },
        {
          ...
          children: (
            <>
              ...
              <p>基本の順番は<strong>画像を追加 → レイアウトを選ぶ → 各画像を調整 → 署名を調整 → 保存</strong>です。先にレイアウトを選び、プレビューの空きスロットに画像を入れることもできます。元の比率を初期設定にし、Xタイムライン用16:9と2:1も選べます。<Link href="/ja/blog">ガイド</Link>もこの順番で、エディターに表示されるラベルに沿って説明しています。</p>
            </>
          ),
        },
        {
        ...
        },
        {
          ...
          children: (
            <>
              <p>最初は画像を追加し、レイアウトを選び、合成・署名・保存の順に進めると安定します。モバイルで背景削除を使う場合は1枚ずつ処理します。PNGを優先し、Xの5MB制限を超える不透明な結果は高画質JPEGに自動最適化します。</p>
        ...
        },
        {
          ...
          children: (
            <FeatureCoverageTable
              intro="2026年8月26日に確認したレビュービルドを基準に、実際の編集画面で確認した機能と制限です。公開版の状態はリリース後に再確認します。処理時間はブラウザ、端末のメモリ、元ファイルによって変わります。"
              areaLabel="領域"
              supportLabel="対応機能"
              notesLabel="利用基準"
              rows={[
                { area: '入力画像', support: 'AVIF · BMP · GIF · JPG · PNG · WebP', notes: '1ファイル50MB以下、写真は最大16枚。大きな画像はブラウザで最適化される場合があります。' },
                { area: 'レイアウト', support: '横分割 · 縦分割 · 2×2グリッド · 3×3 · 4×4', notes: '2×2グリッドは3～4枚（3枚の場合は1枠が空きます）、3×3・4×4はそれぞれ9枚・16枚を使い、Xタイムライン・元の比率・2:1と間隔・枠・背景を調整します。' },
